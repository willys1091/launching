...
trait GeneralTrait
{
    ...
    //     return $value;
    // }
    public function getMarketplace($id = null, $name = null)
    {
        if ($id != null) {
            return marketplace::where('id', $id)->firstOrFail();
        } elseif ($name != null) {
            return marketplace::where('name', $name)->firstOrFail();
        } else {
            return marketplace::where('status', true)->get();
        }
    }
}
